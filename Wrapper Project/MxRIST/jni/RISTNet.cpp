...
bool RISTNetTools::buildRISTURL(const std::string &lIP, const std::string &lPort, std::string &rURL, bool lListen) {
    ...
    if (isIPv4(lIP)) {
        ...
        __android_log_print(ANDROID_LOG_ERROR, APPNAME, "Provided IP-Address not valid => %s", lIP.c_str());
        return false;
    }
    int32_t lPortNum = 0;
    ...
    if (lPortNum < 1 || lPortNum > UINT16_MAX) {
        __android_log_print(ANDROID_LOG_ERROR, APPNAME, "Provided Port number not valid. => %s", lPort.c_str());
        return false;
    }
    std::string lRistURL{};
    if (lIPType == AF_INET) {
        lRistURL += "rist://";
        __android_log_print(ANDROID_LOG_INFO, APPNAME, "RistURL for Ipv4: %s", lRistURL.c_str());
    } else {
        lRistURL += "rist6://";
        __android_log_print(ANDROID_LOG_INFO, APPNAME, "RistURL for Ipv6: %s", lRistURL.c_str());
    }
    if (lListen) {
        lRistURL += "@";
    }
    ...
    rURL = lRistURL;
    __android_log_print(ANDROID_LOG_INFO, APPNAME, "Final RIST URL. =====> %s", rURL.c_str());
    return true;
}
//---------------------------------------------------------------------------------------------------------------------
//
...
RISTNetReceiver::RISTNetReceiver() {
    ...
    __android_log_print(ANDROID_LOG_INFO, APPNAME, "RISTNetReceiver constructed");
}
...
RISTNetReceiver::~RISTNetReceiver() {
    if (mRistContext) {
        ...
        if (lStatus) {
            __android_log_print(ANDROID_LOG_ERROR, APPNAME, "rist_receiver_destroy failure");
        }
    }
    __android_log_print(ANDROID_LOG_INFO, APPNAME, "RISTNetReceiver destruct");
}
...
std::shared_ptr<RISTNetReceiver::NetworkConnection> RISTNetReceiver::validateConnectionStub(std::string lIPAddress,
    ...
    __android_log_print(ANDROID_LOG_ERROR, APPNAME, "validateConnectionCallback not implemented. Will not accept connection from: %s:%d", lIPAddress.c_str(), lPort);
...
int RISTNetReceiver::dataFromClientStub(const uint8_t *pBuf, size_t lSize,
    ...
    __android_log_print(ANDROID_LOG_ERROR, APPNAME, "networkDataCallback not implemented. Data is lost");
...
int RISTNetReceiver::receiveData(void *pArg, rist_data_block *pDataBlock) {
    ...
    if (netObj != lWeakSelf->mClientListReceiver.end()) {
        ...
        __android_log_print(ANDROID_LOG_ERROR, APPNAME, "receive send Data mClientListReceiver <-> peer mismatch.");
    }
    return -1;
}
...
int RISTNetReceiver::clientDisconnect(void *pArg, rist_peer *pPeer) {
    ...
    if (netObj == lWeakSelf->mClientListReceiver.end()) {
        __android_log_print(ANDROID_LOG_ERROR, APPNAME, "RISTNetReceiver::clientDisconnect unknown peer");
        return 0;
    }
...
bool RISTNetReceiver::closeClientConnection(rist_peer *lPeer) {
    ...
    if (netObj == mClientListReceiver.end()) {
        __android_log_print(ANDROID_LOG_ERROR, APPNAME, "Could not find peer");
        return false;
    }
    mClientListReceiver.erase(lPeer);
    int lStatus = rist_peer_destroy(mRistContext, lPeer);
    if (lStatus) {
        __android_log_print(ANDROID_LOG_ERROR, APPNAME, "rist_receiver_peer_destroy failed:");
...
bool RISTNetReceiver::destroyReceiver() {
    if (mRistContext) {
        ...
        __android_log_print(ANDROID_LOG_WARN, APPNAME, "rist_receiver_peer_destroy failed:");
...
bool RISTNetReceiver::initReceiver(std::vector<std::string> &rURLList,
    ...
    if (rURLList.empty()) {
        __android_log_print(ANDROID_LOG_INFO, APPNAME, "URL list is empty.");
        return false;
    }
    ...
    if (lStatus) {
        __android_log_print(ANDROID_LOG_ERROR, APPNAME, "rist_logging_set failed.");
        return false;
    }
    ...
    if (lStatus) {
        __android_log_print(ANDROID_LOG_ERROR, APPNAME, "rist_receiver_create failed.");
        return false;
    }
    for (auto &rURL: rURLList) {
        ...
        if (lStatus)
        {
            __android_log_print(ANDROID_LOG_ERROR, APPNAME, "rist_parse_address fail: %lu", rURLList.size());
            destroyReceiver();
            return false;
        }
        ...
        if (lStatus) {
            __android_log_print(ANDROID_LOG_ERROR, APPNAME, "rist_receiver_peer_create fail: %lu", rURLList.size());
            destroyReceiver();
            return false;
        }
    }
    ...
    if (rSettings.mMaxjitter) {
        ...
        if (lStatus) {
            __android_log_print(ANDROID_LOG_ERROR, APPNAME, "rist_receiver_jitter_max_set fail.");
    ...
    if (lStatus) {
        __android_log_print(ANDROID_LOG_ERROR, APPNAME, "rist_receiver_oob_set fail.");
        destroyReceiver();
        return false;
    }
    ...
    if (lStatus) {
        __android_log_print(ANDROID_LOG_ERROR, APPNAME, "rist_receiver_data_callback_set fail.");
    ...
    if (lStatus) {
        __android_log_print(ANDROID_LOG_ERROR, APPNAME, "rist_receiver_auth_handler_set fail.");
    ...
    if (lStatus) {
        __android_log_print(ANDROID_LOG_ERROR, APPNAME, "rist_receiver_start fail.");
        destroyReceiver();
        return false;
    }
    return true;
...
bool RISTNetReceiver::sendOOBData(rist_peer *pPeer, const uint8_t *pData, size_t lSize) {
    if (!mRistContext) {
        __android_log_print(ANDROID_LOG_ERROR, APPNAME, "RISTNetReceiver not initialised.");
        return false;
    }
    ...
    if (lStatus) {
        __android_log_print(ANDROID_LOG_ERROR, APPNAME, "rist_receiver_oob_write failed.");
...
RISTNetSender::RISTNetSender() {
    validateConnectionCallback = std::bind(&RISTNetSender::validateConnectionStub, this, std::placeholders::_1,
                                           std::placeholders::_2);
    __android_log_print(ANDROID_LOG_INFO, APPNAME, "RISTNetSender constructed");
}
...
RISTNetSender::~RISTNetSender() {
    if (mRistContext) {
        int lStatus = rist_destroy(mRistContext);
        if (lStatus) {
            __android_log_print(ANDROID_LOG_ERROR, APPNAME, "rist_sender_destroy fail.");
        }
    }
    __android_log_print(ANDROID_LOG_INFO, APPNAME, "RISTNetClient destruct.");
}
...
std::shared_ptr<RISTNetSender::NetworkConnection> RISTNetSender::validateConnectionStub(const std::string &ipAddress,
    ...
    __android_log_print(ANDROID_LOG_ERROR, APPNAME, "validateConnectionCallback not implemented. Will not accept connection from: %s:%d", ipAddress.c_str(), port);
...
int RISTNetSender::clientDisconnect(void *pArg, rist_peer *pPeer) {
    ...
    if (netObj == lWeakSelf->mClientListSender.end()) {
        __android_log_print(ANDROID_LOG_ERROR, APPNAME, "RISTNetSender::clientDisconnect unknown peer");
        return 0;
    }
...
bool RISTNetSender::closeClientConnection(rist_peer *lPeer) {
    ...
    int lStatus = rist_peer_destroy(mRistContext, lPeer);
    if (lStatus) {
        __android_log_print(ANDROID_LOG_ERROR, APPNAME, "rist_sender_peer_destroy failed:");
...
bool RISTNetSender::destroySender() {
    if (mRistContext) {
        ...
        __android_log_print(ANDROID_LOG_WARN, APPNAME, "RIST Sender not running.");
...
bool RISTNetSender::initSender(std::vector<std::tuple<std::string,int>> &rPeerList,
    ...
    if (rPeerList.empty()) {
        ...
        __android_log_print(ANDROID_LOG_ERROR, APPNAME, "Sender: URL list is empty.");
    ...
    if (lStatus) {
        __android_log_print(ANDROID_LOG_ERROR, APPNAME, "rist_sender_create fail.");
    ...
    for (auto &rPeerInfo: rPeerList) {
        ...
        if (lStatus)
        {
            ...
            __android_log_print(ANDROID_LOG_ERROR, APPNAME, "rist_parse_address fail:");
        ...
        if (lStatus) {
            ...
            __android_log_print(ANDROID_LOG_ERROR, APPNAME, "rist_sender_peer_create fail:");
    ...
    if (rSettings.mMaxJitter) {
        lStatus = rist_jitter_max_set(mRistContext, rSettings.mMaxJitter);
        if (lStatus) {
            __android_log_print(ANDROID_LOG_ERROR, APPNAME, "rist_sender_jitter_max_set fail.");
    ...
    lStatus = rist_oob_callback_set(mRistContext, receiveOOBData, this);
    if (lStatus) {
        __android_log_print(ANDROID_LOG_ERROR, APPNAME, "rist_sender_oob_set fail.");
    ...
    lStatus = rist_auth_handler_set(mRistContext, clientConnect, clientDisconnect, this);
    if (lStatus) {
        __android_log_print(ANDROID_LOG_ERROR, APPNAME, "rist_sender_auth_handler_set fail.");
    ...
    lStatus = rist_start(mRistContext);
    if (lStatus) {
        __android_log_print(ANDROID_LOG_ERROR, APPNAME, "rist_sender_start fail.");
...
bool RISTNetSender::sendData(const uint8_t *pData, size_t lSize, uint16_t lConnectionID) {
    if (!mRistContext) {
        __android_log_print(ANDROID_LOG_ERROR, APPNAME, "RISTNetSender not initialised.");
        return false;
    }
    ...
    if (lStatus < 0) {
        __android_log_print(ANDROID_LOG_ERROR, APPNAME, "rist_client_write failed.");
        destroySender();
        return false;
    }

    if (lStatus != lSize) {
        __android_log_print(ANDROID_LOG_ERROR, APPNAME, "Did send %d bytes, out of %d bytes", lStatus, unsigned (lSize));
        return false;
    }
...
bool RISTNetSender::sendOOBData(rist_peer *pPeer, const uint8_t *pData, size_t lSize) {
    ...
    int lStatus = rist_oob_write(mRistContext, &myOOBBlock);
    if (lStatus) {
        __android_log_print(ANDROID_LOG_ERROR, APPNAME, "rist_sender_oob_write failed.");
